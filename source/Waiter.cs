...
namespace yrewind
{
    ...
    class Waiter
    {
        ...
        // XML-wrapped JSON created from stream HTML page
            ...
            while (true)
            {
                ...
                if (JsonHtml.XPathSelectElement("//isLiveNow").Value == "true")
                {
                    if (JsonHtml.XPathSelectElement("//targetDurationSec") != null)
                    {
                        ...
                        // Sometimes direct video URL from JSON returns HTTP error 403, so we need to check it
                ...
                if (JsonHtml.XPathSelectElement("//isUpcoming") != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            var start = DateTime.ParseExact(Validator.Start, "yyyyMMdd:HHmmss", null);
        ...
        #region GetHtmlJson - Get HTML page and JSON object from it
        int GetHtmlJson()
        {
            var content = string.Empty;
            ...
            if (Validator.Log) Program.Log(content, "html_json");
