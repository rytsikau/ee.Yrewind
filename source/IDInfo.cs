...
namespace yrewind
{
    ...
    class IDInfo
    {
        ...
        // Stream start point
        // (assumes that there are no interrupts caused by network errors,
        // so it not equal to the one declared in the 'get_video_info' page)
        ...
        int UseBrowser()
        {
            ...
            try
            {
                ...
                var curSeqUtcStr = Regex.Match(segment,
                    @".*Capture-Walltime-Us: (\d+).*").Groups[1].Value;
