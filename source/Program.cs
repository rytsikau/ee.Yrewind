    ...
    static class Program
    {
        ...
        static void Main()
        {
            ...
            while (true)
            {
                ...
                else if ((Preparer.Start - (Program.Start + Program.Timer.Elapsed)).TotalHours > 1)
                {
                    // If determined start point is in future, and more than an hour before start,
                    // it's better to run Preparer again after waiting
                    Program.CountdownTimer
                        ((Preparer.Start - (Program.Start + Program.Timer.Elapsed)).TotalSeconds);
                }
                else if ((Preparer.Start - (Program.Start + Program.Timer.Elapsed)).TotalSeconds > 10)
                {
                    Program.CountdownTimer
                        ((Preparer.Start - (Program.Start + Program.Timer.Elapsed)).TotalSeconds);
                    break;
            ...
            {
                // set default like '9Auq9mYxFEE_20201231-073000_060.00m_1080p'
            ...
            int timeToWait;
            if (Preparer.Start < (Program.Start + Program.Timer.Elapsed))
            {
                timeToWait = Constants.RealTimeBufferSeconds -
                    (int)((Program.Start + Program.Timer.Elapsed) - Preparer.Start).TotalSeconds;
            }
            else
            {
                timeToWait = Constants.RealTimeBufferSeconds -
                    (int)(Preparer.Start - (Program.Start + Program.Timer.Elapsed)).TotalSeconds;
            }

            if (timeToWait > 0)
            {
                // "Waiting for cached data"
                Console.WriteLine(Constants.Msg[9056].ToLower() + "...");
        ...
        static string ReplaceWildcards(string value)
        {
            ...
            // To convert [seconds] to [minutes].[seconds]
            var duration = TimeSpan.FromSeconds(Preparer.Duration);
            var durationStr = string
                .Format("{0:D3}.{1:D2}", (int)duration.TotalMinutes, duration.Seconds);
            ...
            if (value.Contains("~~~asterisk~~~getnext~~~asterisk~~~"))
            {
                ...
                {
                    getnext = getnext
                        .Replace("*getnext*", "*getnext* -l=\"" + Constants.LogWord + "\"");
                }
