...
namespace yrewind
{
    ...
    static class Program
    {
        ...
        // the start time point is determined accurately (using UTC in the stream technical info)
        ...
        static void Main()
        {
            ...
            while (true)
            {
                ...
                var secondsToWait = (long)((Preparer.Start - (Program.Start +
                    Program.Timer.Elapsed)).TotalSeconds + Constants.RealTimeBuffer);
            ...
            {
                // Set default like 'oJUvTVdTMyY_20201231-073000_060m00s_0720p'
        ...
        public static void CountdownTimer(long seconds)
        {
            if (seconds < 0) return;
        ...
        #region LogClean - Hide user IP and cookie from log files
            ...
            foreach (var logFile in Directory.GetFiles(Program.LogDir))
            {
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(userIp))
                        content = content.Replace(userIp, "[" + userIp.Length + "_bytes]");
                    if (!string.IsNullOrEmpty(userCookies))
                        content = content.Replace(userCookies, "[" + userCookies.Length + "_bytes]");
