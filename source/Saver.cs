...
namespace yrewind
{
    ...
    class Saver
    {
        ...
        public int Common()
        {
            ...
            {
                int ffmpegExitCode;
                if (Validator.Browser == default)
                    { code = CreateMedia(outputPathTmp, out ffmpegExitCode); }
                else
                    { code = FixCreateMedia(outputPathTmp, out ffmpegExitCode); }
        ...
        #region FixCreateMedia - Patch to fix YouTube throttling
        int FixCreateMedia(string outputPath, out int ffmpegExitCode)
        {
            ffmpegExitCode = 0;

            var uriAdirect = Regex.Replace(Preparer.UriAdirect, @"&ump=\d+", "");
            var uriVdirect = Regex.Replace(Preparer.UriVdirect, @"&ump=\d+", "");

            // Sometimes video direct URL isn't really direct 
            var reallyDirect = false;
            try
            {
                while (!reallyDirect)
                {
                    ...
                    var res = (HttpWebResponse)req.GetResponse();
                    reallyDirect = res.ContentType.Contains("video");
            ...
            for (var i = seqStart; i <= seqStop; i++)
            {
                contentA += uriAdirect + "&sq=" + i + "\n";
                contentV += uriVdirect + "&sq=" + i + "\n";
            }

            Console.WriteLine("downloading data...");
            if (Preparer.Resolution > 0) FixDownloadM3U(contentV, outputPathTmp + ".video");
            FixDownloadM3U(contentA, outputPathTmp + ".audio");

            Console.WriteLine("concatenating media files...");
            ...
            // -loglevel quiet/panic/fatal/error/warning/info/verbose/debug
            var videoSource = "";
            if (Preparer.Resolution > 0) videoSource = "-i " + outputPathTmp + ".video";
            var arguments =
                "-loglevel fatal" + " " +
                "-stats" + " " +
            ...
            try
            {
                ...
                File.Delete(outputPathTmp + ".video");
                File.Delete(outputPathTmp + ".audio");
            }
            catch
            {
                // Ignore
        ...
        #region FixDownloadM3U - Patch to fix YouTube throttling
        void FixDownloadM3U(string content, string path)
        {
            byte[] segment = default;
            double downloadSpeed;
            double downloadedBytes = default;
            ...
            using (var fileStream = new FileStream(path, FileMode.Append, FileAccess.Write))
            {
                using (var bw = new BinaryWriter(fileStream))
                {
                    foreach (var line in content.Split('\n'))
                    {
                        ...
                        {
                            segment = wc.DownloadData(line);
                        }
                        bw.Write(segment);
                        bw.Flush();
                        fileStream.Flush();

                        downloadedBytes += segment.Length;
                        downloadSpeed = downloadedBytes / (DateTime.Now - time).TotalMilliseconds / 1000 * 8;
                        perc = segmentCnt++ * 100 / contentLength;
                        Console.Write("\r    {0}% at {1:0.000} Mbps", perc, downloadSpeed);
                    }
                }
            }
