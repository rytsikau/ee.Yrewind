...
namespace yrewind
{
    ...
    class Preparer
    {
        ...
        #region CorrectUriVdirect_Browser - Get [UriVdirect] with required 'itag' from browser netlog
        ...
        int FindStart()
        {
            ...
            else if (Waiter.IdStatus == Waiter.Stream.Ongoing)
            {
                ...
                // so we are forced to trim the unstable interval 'with a margin'
            ...
            // Calculate sequence number of corrected requested point
            // (round up to the beginning of the next sequence)
            var seqX = (int)Math.Ceiling((Start - IdStart).TotalSeconds / SeqDuration);

            // The moment when sequence becomes available
            var seqAv = (Program.Start + Program.Timer.Elapsed)
                .AddSeconds(Constants.RealTimeBufferSeconds);

            // Determine [StartSeq] if it should be available
            if ((Start > seqAv) && !FindStart_CheckSeq(seqX))
            {
                // "Requested time interval isn't available"
                Program.ErrInfo = new StackFrame(0, true).GetFileLineNumber() + "";
                return 9313;
