...
namespace yrewind
{
    ...
    class Cache
    {
        ...
        public bool Read(string id, out string idStatus, out string channelId, out string jsonHtmlStr)
        {
            ...
            {
                var lineId = line.Split(new string[] { "~~~" }, StringSplitOptions.None)[1];

                if (id.Trim() == lineId.Trim())
                {
                    idStatus = line.Split(new string[] { "~~~" }, StringSplitOptions.None)[2];
                    channelId = line.Split(new string[] { "~~~" }, StringSplitOptions.None)[3];
                    jsonHtmlStr = line.Split(new string[] { "~~~" }, StringSplitOptions.None)[4];
                }
            }
        ...
        string[] GetContent()
        {
            ...
            try
            {
                ...
                foreach (var line in File.ReadAllLines(Constants.PathCache))
                {
                    ...
                    var value = line.Split(new string[] { "~~~" }, StringSplitOptions.None)[0];
