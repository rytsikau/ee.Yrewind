...
namespace yrewind
{
    ...
    static class Program
    {
        ...
        static string updateZipUri;     // Direct URL of update
        ...
        static void Main()
        {
            ...
            // Determine if the program should work
            if (code != 0) ExitProgram(code); // CheckForUpdates() returned an error
            DateTime.TryParseExact(
                Constants.ExpiryDate.ToString(), "yyMMdd", null, 0, out var expiryDate);
            if ((updateExists & updateCritical) | (expiryDate < Start))
            {
            ...
            if (timeToWait > 0)
            {
                Console.WriteLine(Constants.Msg[9035] + "..."); // "caching data, wait a minute"
